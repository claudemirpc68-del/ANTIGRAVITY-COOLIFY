...
def detect_test_framework(project_path: Path) -> dict:
    ...
    # Python project
    if (project_path / "pyproject.toml").exists() or (project_path / "requirements.txt").exists() or (project_path / "tests").exists():
        result["type"] = "python"
        ...
        if venv_scripts.exists():
            python_cmd = str(venv_scripts / "python")
        
        result["cmd"] = [python_cmd, "-m", "pytest", "-v"]
        result["coverage_cmd"] = [python_cmd, "-m", "pytest", "--cov", "--cov-report=term-missing"]
