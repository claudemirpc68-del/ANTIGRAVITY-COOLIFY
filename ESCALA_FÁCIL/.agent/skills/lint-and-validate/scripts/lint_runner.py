...
def detect_project_type(project_path: Path) -> dict:
    ...
    # Python project
    if (project_path / "pyproject.toml").exists() or (project_path / "requirements.txt").exists() or (project_path / ".venv").exists():
        result["type"] = "python"
        
        # Determine python linter commands
        ruff_cmd = "ruff"
        mypy_cmd = "mypy"
        
        # Check for venv
        venv_scripts = None
        if (project_path / ".venv" / "Scripts").exists():
            venv_scripts = project_path / ".venv" / "Scripts"
        elif (project_path / "venv" / "Scripts").exists():
            venv_scripts = project_path / "venv" / "Scripts"
        elif (project_path / ".venv" / "bin").exists():
            venv_scripts = project_path / ".venv" / "bin"
            
        if venv_scripts:
            if (venv_scripts / "ruff.exe").exists() or (venv_scripts / "ruff").exists():
                ruff_cmd = str(venv_scripts / "ruff")
            if (venv_scripts / "mypy.exe").exists() or (venv_scripts / "mypy").exists():
                mypy_cmd = str(venv_scripts / "mypy")
        
        # Check for ruff
        result["linters"].append({"name": "ruff", "cmd": [ruff_cmd, "check", ".", "--exclude", ".venv,.agent,tests"]})
